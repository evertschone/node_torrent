...
import type { QBittorrentConnectionSettings } from './clientConnectionSettings';
...
interface PiecesInfo { pieces: number[] }
...
class ClientRequestManager {
  ...
  async updateAuthCookie(connectionSettings?: QBittorrentConnectionSettings): Promise<void> {
    ...
    if (authFailed) {
      console.log("Error, QBT Authfailed")
      // throw new Error();
    }
  }
  ...
  async getTorrentInfos(options: null | GetTorrentInfoOptions = null): Promise<QBittorrentTorrentInfos> {
    const form: { [key: string]: string } = {};
    if (options) {
      Object.keys(options).forEach((key) => {
        const property = key as keyof typeof options;
        form[property] = `${options[property]}`;
      });
    }

    return axios
      .post<QBittorrentTorrentInfos>(`${this.apiBase}/torrents/info`, options ? new URLSearchParams(form) : null, {
        headers: await this.getRequestHeaders(),
      })
      .then((res) => res.data);
  }
  ...
  async getPieceStates(hash: string): Promise<PiecesInfo> {
    return axios
      .post<PiecesInfo>(`${this.apiBase}/torrents/pieceStates`,
        new URLSearchParams({
          hash: hash.toLowerCase(),
        }), {
        headers: await this.getRequestHeaders(),
